...
- Load the **Architecture** Google Drive file from Inception → Reverse Engineering
- Load the **Component Inventory** Google Drive file from Inception → Reverse Engineering
- Load the **Technology Stack** Google Drive file from Inception → Reverse Engineering
...
Analyze whatever the user has provided:
   - Intent statements or descriptions (already logged in audit)
...
### Step 6: Generate Clarifying Questions (PROACTIVE APPROACH)
   - **ALWAYS** create a **Requirement Verification Questions** Google Drive file under Inception → Requirements unless requirements are exceptionally clear and complete
...
DO NOT proceed to Step 7 until all questions in the Requirement Verification Questions file are answered and validated.
...
### Step 7: Generate Requirements Document
   ...
   - Create a **Requirements** Google Drive file under Inception
...
```markdown
> **📋 <u>**REVIEW REQUIRED:**</u>**  
> Please examine the requirements in Google Drive under Inception → Requirements
...
**Note**: Include the "Add User Stories" option only when User Stories stage will be skipped. Replace [User Stories/Workflow Planning] with the actual next stage name.
   ...
   - Update Requirements Analysis stage complete in aidlc-state Google Drive file
